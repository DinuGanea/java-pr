...
import wikiXtractor.extractors.MonumentPropertiesExtractor;
...
/**
 * For each monument node in the database, extract valuable information
 * <p>
 * MonumentExtraction <<db-directory>>
 *
 * @author Sonia Rooshenas
 */
@Parameters(commandDescription = "Extract information about a monument")
public class MonumentExtractionCommand extends CLICommand<Void> {

    // command name
    private static final String NAME = "MonumentExtraction";
    ...
    public Void execute() throws Exception {
        ...
        // Get an instance of the monument mapper
        MonumentMapper mapper = MonumentMapper.getInstance();
        ...
        try {
            ...
            logger.info(String.format("%d content entities of type 'Monuments' loaded", monuments.size()));
            ...
            for (Monument m : monuments.values()) {
                ...
                // Form the actual content, needed to search into. These are the categories. They contain the most valuable
                // information about a monument
                StringBuilder cats = new StringBuilder();
                for(String cat : m.getSourcePage().getCatTitles()) {
                    cats.append(cat).append("\n");
                }

                // Extract information about the monument
                HashMap<String, String> properties = MonumentPropertiesExtractor.extract(cats.toString());
                properties.put(MonumentMapper.NAME, m.getRawTitle());

                // Assigne each property to it's attribute
                mapper.map(m, properties);

                // Extract all hyperlinks
                HashMap<String, String> links = LinkExtraction.extractLinks(m.getSourcePage().getHtmlContent(), PageFactory.LINKS_SELECTOR, m.getSourcePage().getPageTitle());

                // If the link matches the raw title of the object in the list, we'll create a reference
                for (String link : links.keySet()) {
                    references++;
                    if (monuments.containsKey(link)) {
                        m.addReferral(monuments.get(link), ContentEntity.MONUMENT_NODE);
                    } else if (persons.containsKey(link)) {
                        m.addReferral(persons.get(link), ContentEntity.PERSON_NODE);
                    } else if (cities.containsKey(link)) {
                        m.addReferral(cities.get(link), ContentEntity.CITY_NODE);
                    } else {
                ...
                if (queriesToExecute % 2000 == 0) {
                    logger.info("{} monument objects updated to database with a total of {} references", queriesToExecute, references);
